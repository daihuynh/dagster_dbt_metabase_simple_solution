...
| `imdb/`       | A Python directory that contains code for your Dagster repository                 |
| `imdb_tests/` | A Python directory that contains tests for `imdb`                      |
...
pyenv virtualenv $PYTHON_VERSION imdb
pyenv activate imdb
```
...
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name imdb python=PYTHON_VERSION
conda activate imdb
```
...
Tests can be found in `imdb_tests` and are run with the following command:

```bash
pytest imdb_tests
```

As you create Dagster ops and graphs, add tests in `imdb_tests/` to check that your
